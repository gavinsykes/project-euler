...
from file_operations import file_exists, get_file_contents
...
def get_gists():
  ...
  problems = {'1':copy(defaultdict)}
  for gist in gists:
    snippets = gist['files']
    for f, d in snippets.items():
      if (findall('euler_problem_',d['filename'])):
        if (findall(r'\d+',d['filename'])):
          prob_num = findall(r'\d+',d['filename'])[0]
        else:
          prob_num = 0
        language = d['language']
        if prob_num not in problems.keys():
          problems[str(prob_num)] = copy(defaultdict)
...
def gists_exist_for_problem(problem_number):
  for gist in gists:
    ...
    for f, d in files.items():
      if (findall(r'\d+',d['filename'])):
        if (findall(r'\d+',d['filename'])[0] == str(problem_number)):
          return True
  return False
...
def gistexists(problem,language):
  for gist in gists:
    ...
    for f, d in files.items():
      if (findall(r'\d+',d['filename'])):
        if (findall(r'\d+',d['filename'])[0] == str(problem)):
...
def fileexists(problem,language):
  try:
    chdir(workingdirectory + '/problem_' + str(problem))
...
def getfiles():
  ...
  for directory in directorylist:
    chdir(workingdirectory + '/' + directory)
    for f in listdir():
